...
def advise(req: AdviseRequest):
    ...
    # 8) LLM (optional) to prettify "why" messages via Ollama (kept short for latency)
    def explain(c: Course) -> str:
        return f"Targets {', '.join(set([planner.parse_skill_tag(s)[0] for s in c.skills]) & set(gap_map.keys()))} and builds toward {req.profile.goal_role}."

    plan_items = []
    citations = planner.make_citations(reranked)
    for c in chosen:
        plan_items.append({"course_id": c.course_id, "why": explain(c), "citations": citations})
